...
function getDates(){
  ...
  load(S.courtDates).filter(x=>!x.done&&x.date).forEach(x=>{const cs=cases[x.caseId]||{},c=clients[x.clientId||cs.clientId]||{};r.push({caseId:x.caseId,date:x.date,kind:'Court',title:x.type||x.note||'Court date',client:cname(c)})});
  load(S.legalDates).filter(x=>!x.done&&x.date).forEach(x=>{const cs=cases[x.caseId]||{},c=clients[x.clientId||cs.clientId]||{};r.push({caseId:x.caseId,date:x.date,kind:'Appointment',title:x.type||x.note||'Appointment',client:cname(c)})});
...
async function migrateLegacyDatesFix8(cs){
  ...
  if(cs.nextLegalDate&&!load(S.legalDates).some(x=>x.caseId===cs.id)){
    const rows=load(S.legalDates);rows.push({id:uid('legal'),caseId:cs.id,clientId:cs.clientId,date:cs.nextLegalDate,type:'',note:cs.nextLegalDateNote||'',done:false,completionNote:'',createdAt:now(),completedAt:null});await save(S.legalDates,rows);changed=true;
  }
  return changed;
}
...
caseDetailInit=function(){
  ...
  const cf=document.getElementById('courtDateForm');if(cf)cf.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(cf),rows=load(S.courtDates);rows.push({id:uid('court'),caseId:id,clientId:cs.clientId,date:String(f.get('date')||''),time:String(f.get('time')||''),type:String(f.get('type')||''),note:String(f.get('note')||''),done:false,completionNote:'',createdAt:now(),completedAt:null});await save(S.courtDates,rows);cf.reset();renderCourtDatesFix8(id);toast('Court date added')});
  const lf=document.getElementById('legalDateForm');if(lf)lf.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(lf),rows=load(S.legalDates);rows.push({id:uid('legal'),caseId:id,clientId:cs.clientId,date:String(f.get('date')||''),type:String(f.get('type')||''),note:String(f.get('note')||''),done:false,completionNote:'',createdAt:now(),completedAt:null});await save(S.legalDates,rows);lf.reset();renderLegalDatesFix8(id);toast('Appointment added')});
